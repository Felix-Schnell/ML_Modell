...
with open("requirements_collab.txt", "w") as f:
    for paket in pakete:
        try:
            ...
            print(f"❌ {paket} nicht gefunden: {e}")
...
print(f"✅ Feature-Namen gespeichert: feature_names.pkl (insgesamt {len(final_feature_names)} Features)")
# =========================================================================
...
print("Bitte die Datei 'df_model_ready_test.csv' hochladen.")
uploaded = files.upload()
test_file_name = list(uploaded.keys())[0]
df_test = pd.read_csv(test_file_name)

print("\nVerarbeite Testdaten mit den trainierten Modellen...")

# === 1. Daten für das Modell vorbereiten
X_test_prepared = pd.DataFrame(columns=feature_names).fillna(0)
X_test_prepared = pd.concat([X_test_prepared, df_test], ignore_index=True, sort=False).fillna(0)
X_test_prepared = X_test_prepared[feature_names]

# === 2. Features aufteilen, skalieren und Autoencoder anwenden ===
# `scaler` und `encoder` existieren bereits im Speicher des Notebooks
base_features_cols = [f for f in feature_names if not f.startswith("ae_feat_")]
ae_features_cols = [f for f in feature_names if f.startswith("ae_feat_")]

X_base = X_test_prepared[base_features_cols]
X_base_scaled = scaler.transform(X_base)
ae_features_predicted = encoder.predict(X_base_scaled)

# === 3. Finale Matrix für XGBoost erstellen ===
# `model` existiert bereits aus dem Training
X_input = pd.DataFrame(X_base_scaled, columns=base_features_cols)
X_input[ae_features_cols] = ae_features_predicted
X_input = X_input[feature_names]

# === 4. Vorhersagen treffen ===
probabilities = model.predict_proba(X_input)[:, 1]
predictions = (probabilities > 0.5).astype(int)
print("Vorhersagen wurden getroffen.")

# === 5. Ergebnisse speichern ===
output_df = df_test.copy()
output_df['fraud_probability'] = probabilities
output_df['is_fraud_prediction'] = predictions

# Wichtige Spalten für den Export auswählen
columns_to_export = [
    'id', 'total_amount', 'transaction_duration_seconds', 'n_lines',
    'was_voided_sum', 'camera_certainty_mean', 'fraud_probability', 'is_fraud_prediction'
]
...
output_df[columns_to_export].to_csv(output_filename, index=False, sep=';', decimal=',')

print(f"\n✅ Fertig! Ergebnisse wurden in '{output_filename}' gespeichert.")
print("Starte den Download...")

# === 6. Finale CSV-Datei herunterladen ===
